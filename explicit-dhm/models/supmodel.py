...
class Model(object):
  ...
  def __init__(self, n_dim, n_chan, n_out, n_superbatch, opt_alg, opt_params):
    ...
    train_set_x = theano.shared(np.empty((n_superbatch, n_chan, n_dim, n_dim), dtype=theano.config.floatX))
    val_set_x = theano.shared(np.empty((n_superbatch, n_chan, n_dim, n_dim), dtype=theano.config.floatX))

    # create y-variables
    train_set_y = theano.shared(np.empty((n_superbatch,), dtype='int32'))
    val_set_y = theano.shared(np.empty((n_superbatch,), dtype='int32'))
  ...
  def fit(self, X_train, Y_train, X_val, Y_val, n_epoch=10, n_batch=100, logname='run'):
    ...
    alpha = 1.0 # learning rate multiplier
    n_data = len(X_train)
    ...
    for epoch in range(n_epoch):
      ...
      alpha = rampdown(epoch, n_epoch, rampdown_length=50)
