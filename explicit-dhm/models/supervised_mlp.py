...
class SupervisedMLP(object):
  ...
  def __init__(self, n_dim, n_aug, n_dim_x, n_dim_y, n_chan, n_out, n_superbatch, opt_alg, opt_params):
    ...
    train_set_x = theano.shared(np.empty((n_superbatch, n_dim), dtype=theano.config.floatX), borrow=False)
    val_set_x = theano.shared(np.empty((n_superbatch, n_dim), dtype=theano.config.floatX), borrow=False)

    # create y-variables
    train_set_y = theano.shared(np.empty((n_superbatch,), dtype='int32'), borrow=False)
    val_set_y = theano.shared(np.empty((n_superbatch,), dtype='int32'), borrow=False)
    ...
    self.network = self.create_model(X, Y, n_dim, n_aug, n_dim_x, n_dim_y, n_out, n_chan)
    ...
    self.train = theano.function([idx1, idx2, alpha], [loss, acc], updates=updates,
                                 givens={X : train_set_x[idx1:idx2], Y : train_set_y[idx1:idx2]},
  ...
  def create_model(self, X, Y, n_dim, n_aug, n_dim_x, n_dim_y, n_out, n_chan=1):
    n_hidden = 2000
    l_in = lasagne.layers.InputLayer(shape=(None, n_dim))
    ...
    l_hid4 = lasagne.layers.DenseLayer(
            l_hid3_drop, num_units=n_hidden,
            nonlinearity=lasagne.nonlinearities.rectify)

    l_hid4_drop = lasagne.layers.DropoutLayer(l_hid4, p=0.2)
    # l_hid4_drop = l_hid4

    l_out = lasagne.layers.DenseLayer(
            l_hid4_drop, num_units=n_out,
  ...
  def create_objectives(self, deterministic=False):
    ...
    # create predictions
    P = lasagne.layers.get_output(l_out, X)
    P_test = lasagne.layers.get_output(l_out, X, deterministic=True)
  ...
  def fit(self, X_train, Y_train, X_val, Y_val, n_epoch=10, n_batch=100, n_lbl_batch=None, logname='run'):
    """Train the model"""
